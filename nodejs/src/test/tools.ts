...
async function toolsExample() {
  ...
  if (!testAddress || !testFromPrivateKey) {
    ...
    console.error(
      "   STACKS_WALLET_A_ADDRESS and STACKS_WALLET_A_PRIVATE_KEY must be set in .env file"
    );
  ...
  try {
    ...
    if (validateResult.success) {
      console.log(
        `✅ Transfer validation: ${validateResult.data ? "Valid" : "Invalid"}`
      );
    } else {
...
//   console.log("Running tools example...\n");
